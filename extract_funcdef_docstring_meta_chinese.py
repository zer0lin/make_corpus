...
def process_function(node, out_func_decl_fd, out_description_fd, out_bodies_fd, out_meta_fd, input_filename):
    ...
    if doc_str is None or not check_chinese_char(doc_str):
        return
    # 处理字符串中的转义字符
    ...
    meta_str = input_filename + " " + str(node.lineno)
...
def process_module(in_fd, out_func_decl_fd, out_description_fd, out_bodies_fd, out_meta_fd, input_filename):
    ...
    for node in module_ast.body:
        if isinstance(node, ast.FunctionDef):
            process_function(node, out_func_decl_fd, out_description_fd, out_bodies_fd, out_meta_fd, input_filename)
